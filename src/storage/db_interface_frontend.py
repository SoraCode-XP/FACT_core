...
class FrontEndDbInterface(MongoInterfaceCommon):
    ...
    def _get_hid_firmware(self, uid):
        ...
        if firmware is not None:
            part = ' -' if 'device_part' not in firmware or firmware['device_part'] == '' else f" - {firmware['device_part']}"
    ...
    def generic_search(self, search_dict, skip=0, limit=0, only_fo_parent_firmware=False, inverted=False):
        try:
            ...
            error_message = f'could not process search request: {sys.exc_info()[0].__name__} {exception}'
